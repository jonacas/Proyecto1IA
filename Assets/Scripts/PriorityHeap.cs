...
/*public struct Item
{
    ...
    public float Cost, Estimated;
    public Node Route;

    public Item(Node nodo)
    {
        this.Nodo = nodo;
        this.QueuePosition = 0;
        this.Cost = float.PositiveInfinity;
        this.Estimated = float.PositiveInfinity;
        Route = null;
    }
}*/
...
public class PriorityHeap : MonoBehaviour {
    ...
    public void Add(Node item, double priority)
    {
        if (count == data.Length)
            Debug.Log("Heap capacity exceeded");
    ...
    public Node ExtractMin() // Probably THE most important function... Got everything working
    {
      //  Debug.Log("Count = " + Count);
            ...
            count--;
            MoveDown(0);
          //  Debug.Log("Sacado del monticulo el nodo: " + minNode.gameObject.name);
            return minNode;
        //}
       /* else
        {
            Debug.Log("Monticulo vacio");
            return null;
        }*/
    ...
    /// aka DecreaseKey 
    /// </summary>
    public bool DecreasePriority(Node n, double priority)
    {
        int position = n.QueuePosition;
        if (position == Node.NO_ESTA_EN_LISTA_ABIERTOS || position == Node.EN_LISTA_CERRADOS)
            return false;
    ...
    public Node Peek() //Devuelve el minimo sin borrarlo
    {
        return data[0];
    }
    ...
    {
       // Debug.Log("Swap de " + position1 + " y de " + position2);
    ...
    public void TestHeapValidity()
    {
        for (int i = 1; i < count; i++)
            if (priorities[Parent(i)] > priorities[i])
                Debug.Log("Heap violates the Heap Property at position " + i);
    }
    #endregion
}
